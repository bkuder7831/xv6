...
scheduler(void)
{
  ...
  for(;;){
    ...
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){ //QQ
      if(p->state != RUNNABLE) //QQ
        continue; //QQ

      min=p;//QQ
    
      for(p1 = ptable.proc; p1 < &ptable.proc[NPROC]; p1++){//
      	if(p1->state == RUNNABLE && p1->passvalue < min->passvalue){//
      		min=p1;//
      	}//
      ...
      proc = 0;
      //min->state = NOTRUNNING (HOW DO I DO THIS)
    }
    release(&ptable.lock);
}
}
